...
void kkputhz(int x, int y, char* s, int flag, int part, int zs, int ks)
{
	int count = 0;
	while (*s) {  // 遍历字符串，直到遇到字符串结束符'\0'
		if ((*s & 0x80) && (*(s + 1) & 0x80)) {  // 判断是否为GBK编码的汉字
			count++;
			s += 2;  // 跳过2个字节
		}
		else {
	...
	setfillstyle(1,ks);  // 设置填充颜色
	bar(x, y, x + count * part, y + part);  // 绘制矩形背景
	setfillstyle(1, zs);  // 设置填充颜色
	puthz(x, y, s, flag, part, zs);  // 绘制汉字
}